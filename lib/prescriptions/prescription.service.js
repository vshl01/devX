...
import { buildFileReference, loadPrescriptionFile } from './storage.js';
import {
...
export async function uploadPrescription(input) {
  ...
  const prescription = await prisma.prescription.create({
    data: {
      ...
      originalFileReference: buildFileReference(validated.fileName),
  ...
  // Background processing; HTTP returns immediately. `after` rather than a bare
  // floating promise because serverless hosts freeze the invocation once the
  // response is sent, which would strand the pipeline mid-flight.
  after(async () => {
    try {
      await processPrescriptionPipeline(prescription.id, {
        requestId: input.requestId,
      });
    } catch (error) {
...
export async function processPrescriptionPipeline(prescriptionId, context = {}) {
  ...
  try {
    ...
    const buffer = await loadPrescriptionFile(prescriptionId);
